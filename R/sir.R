...
for (i in 1:nrow(parameters)) {
    ...
    for (t in 1:steps) {
        ...
        for (j in 1:N) {
            ...
            if ((health_states_t[j] == "S") && (runif(1) < foi * dt)) {
                # Infect a random individual
                health_states_t[sample.int(n = N, size = 1)] <- "I"
                to_infect <- c(to_infect, j)
            }
            # Recover infected individuals
            if ((health_states_t[j] == "I") && (runif(1) < gamma * dt)) {
                to_recover <- c(to_recover, j)
            }
        }
